...
import com.naszi.mobilapp.to_dolistapp.navigation.Screen
...
fun HomeView(
    ...
    Scaffold(
        ...
        val taskLists = viewModel.getAllTasks.collectAsState(initial = listOf())
        LazyColumn(
            modifier = Modifier
                .fillMaxSize()
                .padding(it)
        ) {
            items(taskLists.value, key = { task -> task.id}) {task ->
